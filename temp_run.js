...
const test_cases = [{"input":"nums = [2,7,11,15], target = 9","expected_output":"[0,1]"}];
...
function main() {
    ...
    for (let tc of test_cases) {
        try {
            ...
            const match = /var\s+([a-zA-Z0-9_]+)\s*=\s*function/g.exec(`
var twoSum = function(nums, target) {
    return [0, 1];
};
`);
            if (!match) {
                const match2 = /function\s+([a-zA-Z0-9_]+)/g.exec(`
var twoSum = function(nums, target) {
    return [0, 1];
};
`);
