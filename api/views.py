...
from .utils import get_leetcode_submissions, validate_code, interview_follow_up, generate_performance_report, generate_interview_questions
...
def compute_all_passed(result):
    """Don't trust the LLM's all_passed — compute it from individual test results."""
    if not result.test_results:
        return False
    return all(tr.passed for tr in result.test_results)
...
class RunCodeView(views.APIView):
    ...
    def post(self, request):
        ...
        result = validate_code(code, language, examples_as_tests, question.description)

        actually_passed = compute_all_passed(result)

        return Response({
            'all_passed': actually_passed,
            'test_results': [tr.model_dump() for tr in result.test_results],
            'error_message': result.error_message,
            'feedback': result.feedback if not actually_passed else '',
        })


class SubmitCodeView(views.APIView):
    """Submit code — validates against ALL test cases and triggers interview if passed"""
    authentication_classes = (CsrfExemptSessionAuthentication, BasicAuthentication)
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request):
        ...
        # Use the same examples as Run for consistency
        # (hidden test_cases generated by LLM can be inconsistent when LLM also validates)
        all_tests = [
            {"input": ex["input"], "expected_output": ex["output"]}
            for ex in (question.examples or [])
        ]
        # Add any additional test_cases that don't duplicate examples
        example_inputs = {ex["input"] for ex in (question.examples or [])}
        ...
        result = validate_code(code, language, all_tests, question.description)
        actually_passed = compute_all_passed(result)

        # Save submission
        submission_status = 'Accepted' if actually_passed else 'Wrong Answer'
...
class FinishInterviewView(views.APIView):
    """Finish a mock interview and generate LLM-based performance report."""
    authentication_classes = (CsrfExemptSessionAuthentication, BasicAuthentication)
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request, mock_id):
        ...
        response = MockInterviewSerializer(mock).data
